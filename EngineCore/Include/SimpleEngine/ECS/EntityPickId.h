 ...
 *
 * GPU 규약:
 * - R32_UINT 텍스처를 f32 0.0f로 clear -> 비트 패턴 0x00000000 = uint 0
 * - 0 = "빈 공간" (Entity 없음)
 * - entity_id + 1 = 유효한 Entity가 기록된 값
 *
 * CPU 측에서 Decode()를 통해 Optional<u32>로 변환하여,
 * miss/hit을 타입 안전하게 구분합니다.
 */
struct EntityPickId
{
    /** GPU clear 값 = 빈 공간. R32_UINT 0.0f clear -> 0x00000000 */
    static constexpr u32 ENCODED_NONE = 0;
...
public:
    ...
    static constexpr EntityPickId Encode(u32 entity_id) { return { entity_id + 1 }; }

    /** GPU readback 원시 값으로부터 결과 생성 */
    static constexpr EntityPickId FromRaw(u32 raw) { return { raw }; }
...
public:
    ...
    [[nodiscard]] constexpr Optional<u32> Decode() const
    {
