...
{
template <typename T>
struct ExtractBaseTypeImpl
{
    using Type = T;
};

template <typename T>
struct ExtractBaseTypeImpl<T*>
{
...
template <typename T>
using ExtractBaseType = ExtractBaseTypeImpl<std::remove_cvref_t<T>>::Type;
}
...
consteval std::string_view RemoveToken(std::string_view signature, std::array<std::string_view, N> token_list) noexcept
{
    ...
    do
    {
        ...
        for (const std::string_view& qualifier : token_list)
        {
...
consteval std::string_view ExtractType_MSVC(std::string_view in_signature) noexcept
{
    ...
    // <>안 Type 정보만 추출
    const std::string_view extracted_typename = string_utils::TrimWhitespace(in_signature.substr(start_pos, end_pos - start_pos));
...
/**
 * 타입의 네임스페이스를 포함한 순수 타입 이름을 컴파일 타임에 추출합니다.
 * 타입에 붙은 모든 수식어(const, volatile, *, &, [])를 제거하고, 네임스페이스가 포함된 기본 타입 이름(예: `int`)을 반환합니다.
 *
 * @tparam T 타입 이름을 추출할 대상 타입
 * @return 네임스페이스를 포함한 순수 타입 이름
 */
template <typename T>
    requires (!se::traits::IsFunctionType<T>)
[[nodiscard]] consteval std::string_view GetFullTypeName() noexcept
{
    using CleanType = details::traits::ExtractBaseType<T>;
    ...
    constexpr auto ret = details::RemoveToken(signature, leading_keywords);
 ...
 * 타입에 붙은 모든 수식어와 네임스페이스를 제거하고, 가장 근본적인 타입 이름(예: `int`)만을 반환합니다.
 *
 * @tparam T 타입 이름을 추출할 대상 타입
