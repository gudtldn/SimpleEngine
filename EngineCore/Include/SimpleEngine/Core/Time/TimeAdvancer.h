...
{
/**
 * ECS Time Resource의 갱신 로직을 담당하는 무상태(stateless) 유틸리티 클래스
 * 데이터를 소유하지 않으며, World가 Resource로 소유하는 시간 객체를 매 프레임 friend 권한으로 갱신합니다.
 */
class SE_CORE_API TimeAdvancer
{
public:
    TimeAdvancer() = delete;
    ...
    /** GameTime을 갱신합니다. pause/time_scale이 적용됩니다. */
    static void AdvanceGameTime(GameTime& game, f64 real_delta);
