...
public:
    ...
    using IteratorType = const ValueType*;
...
public:
    ...
    [[nodiscard]] constexpr const ValueType& operator[](SizeType idx) const noexcept
    {
        return data_ptr[idx];
    }

    /** 인덱스 위치의 문자를 반환합니다. (범위 검사 있음) */
    [[nodiscard]] constexpr Optional<const ValueType&> At(SizeType idx) const noexcept
    {
        if (idx >= data_len)
        {
            return std::nullopt;
    ...
    /** 첫 번째 문자를 반환합니다. */
    [[nodiscard]] constexpr Optional<const ValueType&> Front() const noexcept
    {
        if (data_len == 0)
        {
            return std::nullopt;
    ...
    [[nodiscard]] constexpr const ValueType& FrontChecked() const noexcept
    {
    ...
    /** 마지막 문자를 반환합니다. */
    [[nodiscard]] constexpr Optional<const ValueType&> Back() const noexcept
    {
        if (data_len == 0)
        {
            return std::nullopt;
    ...
    [[nodiscard]] constexpr const ValueType& BackChecked() const noexcept
    {
    ...
    /** 데이터 포인터를 반환합니다. */
    [[nodiscard]] constexpr const ValueType* Data() const noexcept { return data_ptr; }
...
public:
    ...
    /** 특정 문자를 찾습니다. */
    [[nodiscard]] constexpr Optional<SizeType> Find(ValueType c, SizeType pos = 0) const noexcept
    {
    ...
    [[nodiscard]] constexpr Optional<SizeType> RFind(ValueType c, SizeType pos = static_cast<SizeType>(-1)) const noexcept
    {
        if (data_len == 0)
        {
            return std::nullopt;
...
private:
    [[nodiscard]] static constexpr SizeType StrLen(const ValueType* str) noexcept
    {
        SizeType len = 0;
        while (str[len] != '\0')
        {
    ...
    [[nodiscard]] static constexpr int32 Compare(const ValueType* lhs, const ValueType* rhs, SizeType len) noexcept
    {
...
private:
    const ValueType* data_ptr = nullptr;
    SizeType data_len = 0;
};
} // namespace se
