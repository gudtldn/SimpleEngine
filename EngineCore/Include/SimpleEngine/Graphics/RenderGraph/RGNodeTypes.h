...
struct RGPassNode
{
    ...
    Array<u32> write_indices; // 이 패스가 쓰는 리소스 인덱스
    Array<u32> read_indices;  // 이 패스가 읽는 리소스 인덱스
...
struct RGResourceNode
{
     ...
     */
    HashMap<u32, u32> version_to_writer;

    // lifetime 정보 (위상 정렬 후 계산됨)
    u32 first_user_pass_index = std::numeric_limits<u32>::max();
    u32 last_user_pass_index = 0;
};
} // namespace se
