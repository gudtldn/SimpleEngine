...
constexpr i32 Utf8SequenceLen(char lead)
{
    const u8 b = static_cast<u8>(lead);
...
{
    return (static_cast<u32>(c) & 0xFFFFF800u) == 0xD800u;
}
 ...
 */
constexpr char32_t Utf8GetUnsafe(const char* s, i32 i)
{
    const u8 b0 = static_cast<u8>(s[i]);
    if ((b0 & 0x80u) == 0)
    {
    ...
    if ((b0 & 0xE0u) == 0xC0u)
    {
        return static_cast<char32_t>(((b0 & 0x1Fu) << 6) | (static_cast<u8>(s[i + 1]) & 0x3Fu));
    }
    if ((b0 & 0xF0u) == 0xE0u)
    {
        return static_cast<char32_t>(((b0 & 0x0Fu) << 12) | ((static_cast<u8>(s[i + 1]) & 0x3Fu) << 6) | (static_cast<u8>(s[i + 2]) & 0x3Fu));
    }
    return static_cast<char32_t>(((b0 & 0x07u) << 18)
        | ((static_cast<u8>(s[i + 1]) & 0x3Fu) << 12)
 ...
 */
void Utf8NextUnsafe(const char* s, i32& i, char32_t& out_c)
{
...
void Utf8Next(const char* s, i32& i, i32 length, char32_t& out_c)
{
    const u8 b0 = static_cast<u8>(s[i]);
    const i32 seq_len = Utf8SequenceLen(static_cast<char>(b0));
...
void Utf8Prev(const char* s, i32 start, i32& i, char32_t& out_c)
{
    --i;
 ...
 */
void Utf8Append(char* buf, i32& offset, i32 capacity, char32_t c, bool& out_error)
{
...
usize CountCodePointsImpl(StringView view)
{
    ...
    const i32 length = static_cast<i32>(view.ByteLen());
...
Optional<std::pair<char32_t, usize>> DecodeLastCodePoint(StringView view)
{
    ...
    const char* s = view.Data();
    i32 i = static_cast<i32>(view.ByteLen());
    const i32 prev_i = i;
