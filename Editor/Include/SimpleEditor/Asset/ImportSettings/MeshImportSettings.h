...
{
    SE_CLASS_V1(MeshImportSettings, ImportSettingsBase)
...
public:
     ...
     */
    SE_ANNOTATION(=meta::Reflect, =meta::DisplayName<"Combine Meshes">{})
    bool combine_meshes = true;

    /** 노드의 트랜스폼(위치, 회전, 크기)을 버텍스 데이터에 영구적으로 적용(Bake)할지 여부 */
    SE_ANNOTATION(=meta::Reflect, =meta::DisplayName<"Apply Transform">{})
    bool apply_transform = true;

    /** 메쉬 전체에 적용할 크기 배율 */
    SE_ANNOTATION(=meta::Reflect, =meta::Range(0.01f, 1000.0f), =meta::DisplayName<"Global Scale">{})
