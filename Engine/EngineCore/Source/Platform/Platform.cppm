...
struct Platform
{
     ...
     */
    static void SetThreadName(std::thread& thread, const u8string& name);

    /**
     * 현재 실행 중인 스레드의 이름을 설정합니다.
     * @param name 설정할 스레드의 이름입니다.
     */
    static void SetCurrentThreadName(const u8string& name);
     ...
     * @return 스레드 이름
     */
    static u8string GetThreadName(std::thread& thread);

    /**
     * 이 함수가 호출되었던 스레드의 이름을 가져옵니다.
     * @return 스레드 이름
     */
    static u8string GetCurrentThreadName();
};
}
